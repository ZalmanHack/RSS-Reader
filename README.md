...
## Архитектура
### Activities
* `MainActivity.java` - главная активность приложения, отображающая новостные ленты с помощью `ViewPager2` и `TabLayout`. Позволяет:
...
* `SettingsActivity.java` - активность настроек приложения, позволяющая пользователю добавлять и удалять новостные ленты. Доступна из панели инструментов в `MainActivity`. Позволяет:
...
* `ShowItemActivity.java` - активность для отображения полной информации по выбранной ранее новости. Доступно по нажатию на одну из карточек новостных лент. Позволяет:
    * название канала;
    * название новости;
...
* `SplashActivity.java` - активность, отображаемая в момент запуска приложения.
...
### Fragments
* `DeleteDialogFragment.java` - диалоговое окно для удаления канала;     
...
### ViewModels
* `FeedViewModel.java` - архитектурное решение, основанное на паттерне проектирования MVVM;
